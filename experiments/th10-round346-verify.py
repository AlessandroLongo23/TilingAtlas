...
for tid,e in G.items():
    ...
    if abs(T1)<1e-9 or abs(T2)<1e-9: degenerate.append(tid); continue   # drop t1002-type
...
print("dropped degenerate (T1 or T2 = 0):",degenerate)
R346=[r for r in rows if r[5] and r[6]]
sus=[r for r in rows if r[8]]
print("tilings with an incomplete edge-star (artifact-prone):",len(sus))
print("  of those, how many are round-and-{3,4,6}:",sum(1 for r in R346 if r[8]))
print("  their ids if any:",[r[0] for r in R346 if r[8]])
print("round-&-{3,4,6} with COMPLETE stars (trustworthy no-12 detection):",sum(1 for r in R346 if not r[8]),"/",len(R346))
# partition check
rnd_all=[r for r in rows if r[5]]; rnd12=[r for r in rnd_all if 12 in r[7]]; rnd8=[r for r in rnd_all if 8 in r[7]]
print("\npartition of round: total=%d = {3,4,6}:%d + has12:%d + has8:%d + other:%d"%(
    len(rnd_all),len(R346),len(rnd12),len(rnd8),len(rnd_all)-len(R346)-len(rnd12)-len(rnd8)))
print("\n=== MAIN (t1002 excluded, R=3): ROUND & {3,4,6}  s* <= 2k+1 ===")
byk=defaultdict(list)
for r in R346: byk[r[2]].append(r)
tot=0
for k in sorted(byk):
    ss=[r[3] for r in byk[k]]; bnd=2*k+1; nv=sum(1 for s in ss if s>bnd)
    att=[r[0] for r in byk[k] if r[3]==bnd]; tot+=nv
    print("  k=%d: n=%3d  max s*=%2d  2k+1=%2d  violations=%d  attainers=%s"%(k,len(ss),max(ss),bnd,nv,att[:6]))
print("TOTAL violations:",tot)
print("round {3,4,6} max-s* sequence k=1..6:",[max(r[3] for r in byk[k]) for k in sorted(byk)],"  vs 2k+1:",[2*k+1 for k in sorted(byk)])
