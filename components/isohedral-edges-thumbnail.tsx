...
// Static preview of one isohedral edge system, for the library grid and the /play sidebar.
// It draws at the type's DEFAULT parameter point, not the live one: a thumbnail's job is to identify the
...
export function IsohedralEdgesThumbnail({ pattern }: { pattern: IhEdgeRecord }) {
	...
	return (
		<canvas
			...
			aria-label={`Isohedral edge system ${pattern.id}`}
		/>
	);
}
