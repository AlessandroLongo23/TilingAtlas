...
def is_corner(letter, board):
    ...
    return bool(m) and m.group(1) in board["corners"] and int(m.group(2)) == len(board["corners"])
...
def corner_classes(board):
    ...
    for i, (corner, cls) in enumerate(board["sides"]):
        ...
        out[corner] = {cls} if board.get("aspects", 1) == 1 else {cls, prev}
    return out
...
def check_incidence(cert, board):
    ...
    for t in cert["types"]:
        ...
        if len(corners) != want:
            raise DevelopError(f"vertex has {len(corners)} corners, not {want}")
...
def combinatorics(block, board):
    ...
    return {
        ...
        "drawn": "".join("1" if block.drawn[h] else "0" for h in range(n)),
