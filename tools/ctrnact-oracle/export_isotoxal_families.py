#!/usr/bin/env python3
"""Sweep the regular+isotoxal catalog for free-α FAMILIES and export them parametrically.

The isotoxal analog of export_family_cells.py. A convex isotoxal tile cxL-α.β is the exact
counterpart of a star species: one free parameter, α at the @0 corner and β = (360 − 360/n) − α at
the @1 corner, so the δ-coefficient is +1 at @0 and −1 at @1 (vs +1 point / −1 dent for stars). The
whole detection + symbolic-development engine in family_flex.py is reused unchanged (it is generic in
`qeff`, the per-corner-class δ-coefficient); only the corner model, the α-validity interval, and the
polygon side-count extraction differ, and those are the isotoxal versions defined here.

Same output schema as the star families (lib/utils/paramCell.ts ParametricCellData): vertices/basis
are Laurent term lists [m, re, im] meaning Σ_m (re + i·im)·e^{i·m·δ}, δ = (alphaDeg − alpha0Deg)·π/180,
where α is the isotoxal tile's @0 (smaller) angle — the UI slider parameter. D = 24 (15° units).

Usage:
  python3 export_isotoxal_families.py --tables tables/regular-isotoxal-z24 \
      --catalog 1:run-regular-isotoxal-z24-k1/out/pruned \
      --out ../../experiments/results/ctrnact-isotoxal-families.cells.json \
      --log ../../experiments/results/isotoxal-families-export.log
"""
import argparse
import cmath
import glob
...
def species_and_q(tab, cls_used):
    """Isotoxal species present among used corner classes; qvec[class] = +1 at @0 (α), −1 at @1 (β)."""
    species = []
    for c in sorted(set(cls_used)):
        if tab.CLASS_DISP[c].startswith("cx"):
            t = tab.CLASS_TILE[c]
            if t not in species:
                species.append(t)
    qvec = {}
    for c in sorted(set(cls_used)):
        q = [0] * len(species)
        m = CX_CORNER.match(tab.CLASS_DISP[c])
        if m:
            q[species.index(tab.CLASS_TILE[c])] = 1 if int(m.group(2)) == 0 else -1
        qvec[c] = tuple(q)
    return species, qvec
...
def vertex_qsums(tab, vertypeline, species):
    ...
    for corners in ff.vertex_words(vertypeline):
        row = [0] * len(species)
        for cn in corners:
            m = CX_CORNER.match(cn.strip())
            if not m:
                continue
            tile = name_by_tile.get(m.group(1))
            if tile in species:
                row[species.index(tile)] += 1 if int(m.group(2)) == 0 else -1
        rows.append(row)
    return rows


def alpha_units(tab, tile):
    """The α (smaller, @0) interior angle of an isotoxal tile in D-units."""
    for c in range(len(tab.CLASS_DISP)):
        if tab.CLASS_TILE[c] == tile and tab.CLASS_DISP[c].endswith("@0"):
            return tab.CLASS_UNITS[c]
    raise ValueError(f"no @0 corner for tile {tab.TILE_NAME[tile]}")


def beta_units(tab, tile):
    for c in range(len(tab.CLASS_DISP)):
        if tab.CLASS_TILE[c] == tile and tab.CLASS_DISP[c].endswith("@1"):
            return tab.CLASS_UNITS[c]
    raise ValueError(f"no @1 corner for tile {tab.TILE_NAME[tile]}")
...
def corner_records(tab, vertypeline, species, direction):
    ...
    every α-pin of one family shares the key. token abstracts the specific angle (cxL@pos)."""
    ...
    for corners in ff.vertex_words(vertypeline):
        ...
        for cn in corners:
            cn = cn.strip()
            m = CX_CORNER.match(cn)
            if m:
                tile = name_by_tile[m.group(1)]
                pos = int(m.group(2))
                q = (1 if pos == 0 else -1) * sp_dir.get(tile, 0)
                L = int(re.match(r"cx(\d+)", m.group(1)).group(1))
                w.append((f"cx{L}@{pos}", q, disp_units[cn] - q * a0))
            else:
                w.append((cn, 0, None))
...
def delta_interval_units(tab, species, direction):
    """(lo, hi) for δ in D-units: every flexing tile keeps BOTH α and β strictly convex, in (0°, 180°)."""
    lo, hi = -1e9, 1e9
    for t, d in zip(species, direction):
        if d == 0:
            continue
        # α = a0 + d·δ ∈ (0, HALF_D);  β = b0 − d·δ ∈ (0, HALF_D)
        for base, coef in [(alpha_units(tab, t), d), (beta_units(tab, t), -d)]:
...
def poly_n(nm):
    return int(re.match(r"cx(\d+)", nm).group(1)) if nm.startswith("cx") else int(nm)
...
def species_delta_interval(tab, tile):
    """δ-range (in D-units) keeping ONE isotoxal species convex: α = a0 + δ ∈ (0,180), β = b0 − δ ∈ (0,180)."""
    lo, hi = -1e9, 1e9
    for base, coef in [(alpha_units(tab, tile), 1), (beta_units(tab, tile), -1)]:
        b1, b2 = -base / coef, (HALF_D - base) / coef
        l, h = (b1, b2) if coef > 0 else (b2, b1)
        lo, hi = max(lo, l), min(hi, h)
    return lo, hi
...
def analyze_block(tab, vertype, conway):
    ...
    if separable:
        ...
        for c in set(cls):
            vec = [0] * P
            mm = CX_CORNER.match(tab.CLASS_DISP[c])
            if mm:
                for p, s in enumerate(param_species):
                    if tab.CLASS_TILE[c] == species[s]:
                        vec[p] = 1 if int(mm.group(2)) == 0 else -1
            qeff[c] = tuple(vec)
    ...
    for c in set(cls):
        if not tab.CLASS_DISP[c].startswith("cx"):
            continue
...
def emit_family(fid, k, members):
    ...
    polys = [{"n": poly_n(tab.TILE_NAME[tile]), "vertices": [lp_terms_multi(v) for v in verts]}
...
def main():
    ...
    ap.add_argument("--id-prefix", default="ctrnact-isotoxal-family")
    args = ap.parse_args()
    LOG = open(args.log, "a")
    log(f"=== isotoxal family export; tables={args.tables} ===")
    try:
    ...
    for spec in args.catalog:
        ...
        blocks = [(vt, cw) for f in files for vt, cw in ff.read_blocks(f) if "cx" in vt]
        log(f"k={k}: {len(blocks)} isotoxal-bearing blocks in {path}")
        fams, n_pinned = {}, 0
        for vt, cw in blocks:
            t0 = time.time()
            try:
