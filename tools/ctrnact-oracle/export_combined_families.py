...
def family_map_key(rneig, lneig, mirro, glue, cls, tab, param_of_cls, norm_of_cls, P):
    ...
    def label_of(d, perm):
        ...
        info = iso_corner_info(disp)  # cx or star corner — abstract angle, encode parameter index
        if not info:
            return disp
        L = poly_n(tab.TILE_NAME[tab.CLASS_TILE[cls[d]]])
        pt = info[1]
        p = param_of_cls.get(cls[d])
        return (f"i{L}{'p' if pt else 'd'}#{perm[p]}@{norm_of_cls[cls[d]]}" if p is not None
