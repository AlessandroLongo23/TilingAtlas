import math;
import os;
if os.environ.get("EU_QUIET","1") != "0":
  def print(*a, **k):
    return None

# Path for the output directory; must exist before the program is started. You should change it to your desired path.
import sys, glob;
filepathi = os.environ.get("EU_OUT", os.path.join(os.path.dirname(os.path.abspath(__file__)), "out"));
filepathi = filepathi.rstrip("/") + "/";
...
def processfile(filec):
  ...
  if not os.path.exists(filepathi+listfile):
    return
...
KNOWN_FAMILIES = ["34","346","346c","34c","36"]
FAMILY_MODE = os.environ.get("EU_FAMILIES","all")   # "all" = process every family the solver emitted; "known" = author's 5-family shortcut (k>=2)

def famof(fn):
    b = os.path.basename(fn)                          # eusolver_<NN>_<fam>.txt
    return b[len("eusolver_")+3 : -len(".txt")]

def run_level(k):
    global filecodebase
    nn = ("0"+str(k)) if k < 10 else str(k)
    filecodebase = nn
    present = [famof(f) for f in sorted(glob.glob(filepathi + "eusolver_" + nn + "_*.txt"))]
    if k >= 2 and FAMILY_MODE == "known":
        fams = [f for f in KNOWN_FAMILIES if f in present]
        skipped = [f for f in present if f not in KNOWN_FAMILIES]
        if skipped:
            sys.stderr.write("NOTE k=%d 'known' mode skips families %s (assumed all non-simplest)\n" % (k, skipped))
    else:
        fams = present                                # process EVERYTHING (completeness-safe)
    for fam in fams:
        processfile(fam)

KMIN = int(os.environ.get("EU_KMIN","2"))
KMAX = int(os.environ.get("EU_KMAX","12"))
for _k in range(KMIN, KMAX+1):
    run_level(_k)
