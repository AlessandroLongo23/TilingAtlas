...
it survives the pivot: it is contribution 4. The figure is one case of it: t2001's two periods, each a
chain of six unit steps, over the cell they generate. Six is the measured maximum over all 92 tilings
with k <= 3 at k = 2 — this is the extremal case, not a friendly one. If anyone asks why only every
other spoke of the rose is used: the odd powers of zeta_24 exist for the octagon alone, and every other
regular-polygon tiling lives in Z[zeta_12]. -->
