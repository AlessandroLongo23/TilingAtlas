...
3. **Generalize `checkpart`.** Face-cycle validity becomes "spells out a real polygon of the set" (corner angles and edge lengths in the right cyclic order), and the divisibility rule uses each polygon's rotational-symmetry order, not its edge count.
...
5. **The concrete success test.** Does the extended engine reproduce Myers' k=1 and k=2 star counts and per-tiling lists exactly? If yes, realizability held and the cross-reference is delivered. If counts miss, that is where it broke, and #2 stays a cross-reference attempt, not a certification.
