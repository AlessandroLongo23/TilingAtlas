...
| `touch-phone:` | Tailwind (custom variant in `globals.css`) | `(hover: none)` AND phone width. Use it where a bare `[@media(hover:none)]` would also reach touch tablets, which keep the desktop layout. |
...
| `mobileIcon` | modal | Pill icon (a lucide component). Default sliders; `List` for "Contents" and "Browse". |
...
| `mobileTrigger` | modal | `false` hides the floating pill when the page has its own button. Default `true`. |
...
| `SheetBadge` | Count pill on a trigger; nothing at 0. |
| `useSwipeDismiss(ref, onClose, axis)` | Only the drag, for a sheet with its own header (Modal uses it). |
...
| `useDockSheet`, `snapCss`, `PEEK_PX`, `pickSnap`, `SheetGrabHandle` | The dock motion. PageSidebar uses it; reuse only for another dock. `snapCss(snap, half?)` is a snap's resting height as CSS. |
...
and closing a layer any other way removes its entry. A query the page rewrote while the sheet was open
survives. A link followed from inside a layer is a real navigation; the nav menu's links use `replace`
so Back from the new page returns to the old one in one press.

## Other shared pieces

| Piece | Where | Notes |
|---|---|---|
| `Modal` | `components/ui/modal.tsx` | Phone: bottom sheet, full width, rounded top, max 92dvh, body scrolls, grab bar, swipe down and Back close it. `footer` prop: action bar that stays in view while the body scrolls. Use a `Modal` for "info card as a sheet". |
| `Tooltip` | `components/ui/tooltip.tsx` | `tapToOpen`. Default on for rich `content` (InfoDot, option tooltips), off for plain `label`. Tap toggles, tap outside closes. Shortcut keycap hidden on phone. The arrow follows the side the popup lands on (`data-side`), so it stays right when there is no room on `side`. |
...
- `Switch`, `InfoDot`: invisible halo, 44px hit area.
...
- Text floor: `.ta-label`, `text-[10px]`, `text-[10.5px]` and `text-[11px]` render at 12px (`mobile.css`), unless the element has its own `max-md:text-*` or sits in an SVG. Smaller arbitrary sizes (8px, 9px) need a `max-md:text-xs` at the call site. InfoDot text 13px.
- Contrast: `fg-muted` and `fg-disabled` are darker on phone (4.5:1 on every surface incl. the sunken track); desktop tokens unchanged.
...
- `FloatingToolbar`: above `--sheet-offset`, max width screen minus 32px, sideways scroll with snap, 44px tall. A "More" overflow menu is page work.
- `FullscreenToggle`: 44px, `top-3 right-3`.
