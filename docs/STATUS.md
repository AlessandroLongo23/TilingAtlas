...
**192 → 58 rows.** The shelf had been built per solved BLOCK, and the engine enumerates maps: once a side
may be split, the same tiling carries many maps, so 146 T-junction rows were 21 families and 102 of them
were byte-identical to another. Worse, the cone was measured over the split map's edges, including split
points that are a corner of no tile — a mark the tiling cannot see — so 134 of 146 rows offered more
sliders than the tiling has freedom. The first two cards were both the plain rectangle grid under three
sliders, two of which moved the same width.

Standing rules this leaves behind, all enforced by tests in `lib/tilings/length-families.test.ts`:
- **`tiling_key.smooth` before any length system.** False vertices are gauge, not parameters.
- **One row per `family_key`** = canonical map + corner angles + which tiles are held equilateral. The
  map alone merges the hexagon grid with offset squares; angles alone merge unit squares with rectangles.
- **Measure at a GENERIC member, never the developed one.** k, the key and the card all read there; at
  every-edge-1 distinct edges coincide, orbits fuse, and a primitive cell can look like a supercell.
- **One edge is pinned as the unit**, so no slider is pure scale and the printed count is the true one.

Open: the T-junction scan covers blocks whose DEVELOPED member has k ≤ 2 (146 of the 4,486 on disk at
k ≤ 4), and the shelf says so on every card. Running it to k ≤ 4 is ~75 min of emitter time and would
widen the shelf honestly; AL's call.
...
**Done 2026-08-18:** the browse manifest. `CatalogueListPanel` draws a row for an unloaded tier from
a 1.8 KB count file, and clicking it fetches the shard — the 84,424 unreachable entries are reachable.
...
## The base hyperbolic shelf has a board axis at last (2026-08-17)

It was the only large shelf with no sub-axis: /play showed two rows, k=1 (12,168) and k=2 (16,285), and
/library offered no board chips. Now **valence → alphabet → configuration → tilings**, 227 boards
generated by `scripts/emit-hyp-tiling-boards.mjs` under six valence headings. Valence alone measured at a
worst row of 5,581, so the configuration level was added: the biggest cell drops to 876, and 167 of the
227 boards are small enough to skip that level entirely.

- ⚑ It states something the shelf never did: a hyperbolic vertex configuration does NOT determine its
  tiling — 12,168 uniform tilings realise 2,591 configurations, `4.6⁷` alone admitting 147.
- The 27 regular {p,q} tilings now read as `{7,3}` instead of a bare `7`.
- `subOf`'s hyperbolic branch reads the FAMILY STRING and is gated on `source === "hyperbolic"`; without
  the gate it would file the plane's uniform tilings ("3.3.3.3.3.3") onto a hyperbolic board.

## ai2 decoded: the {3,n} family, nine boards on the hyperbolic-poly shelf (2026-08-17)
...
**The /play catalogue can reach every lazy shard (fixed 2026-08-18).** It used to derive its k rows
from loaded tilings only, so the row that would trigger a fetch could never appear. `public/atlas-manifest.json`
(1.8 KB, generated by `scripts/gen-atlas-manifest.ts`) carries counts per (class, sub, k); an unloaded
tier draws a row with its real count and a download glyph, and clicking it loads the shard. Measured
before the fix: **84,424** tilings unreachable — scaled k3–k7 (43,317), regular k8/9/10 (29,163),
euhalf k5–k9 (10,153), mixed k3–k4 (697). The counts are generated by decoding each shard, and
`atlasManifest.test.ts` checks every one against its shard in both directions, so a stale manifest
fails the suite instead of promising a number the shelf cannot deliver.
