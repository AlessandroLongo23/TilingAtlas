...
## 31. CB-7 (primitivity-rejection guard) + CB-8 (pool-reach loud truncation) — the tuned regime is now self-announcing (2026-06-10, session 16f)
...
`6f9ca9cf2d16c75f` (byte-identical) [k=2 digest noted below when the run lands].

### 31.1 CB-8 — poolConfig centralization + regime banner + per-candidate reach checks
...
### 31.2 CB-7 — primitivity-rejection guard (+ two findings made along the way)
...
- k=1 probe digest `6f9ca9cf2d16c75f` count=11 — byte-identical; banner + reach lines on stderr.
  k=2 probe logged to `experiments/results/cb7-cb8-probes-2026-06-10.log`.
