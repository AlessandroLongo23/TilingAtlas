 ...
 *    this test opts into the widen-only pool override (POOL_STEPS_UP/POOL_LMAX_UP, NOTES §35) — a
...
describe("ST-9 — productive star-fill (C3) positive coverage", () => {
	...
	describe("Myers 4(i) 8.3*@1.8.6*@5 — the one fill-requiring in-ring Fig-4 tiling", () => {
		beforeAll(() => {
			// widen-only pool override (NOTES §35): 4(i)'s period is outside the tuned pool — its
