...
import { tilePaletteGlsl } from "@/lib/render/tilePalette";
...
// The shared tile palette, so a spherical face is the same colour as a Euclidean one. sph_-prefixed,
// because this chunk is injected into a three.js MeshStandardMaterial where a bare tileFill could
// collide with three's own code.
${tilePaletteGlsl("sph_")}
...
vec3 sphFaceColor(int best) {
	return sph_tileFill(uSphFace[best].w + uSphHueOffset);
}
`;
