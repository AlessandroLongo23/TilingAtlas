...
export class PeriodSolver {
	...
	private torusFill(corePolys: Polygon[], ctx: FillCtx, timedOut: () => boolean, diag: PeriodSolverDiag): Polygon[][] {
		...
		if (skipP1 && process.env.SPIKE_TRACE === '1') {
			...
			if (ctx.includeDents) {
				// (F5) star fill-reach lemma (star-fill-dentseating-workorder-2026-06-11): the dent loop in
				// the gap-completion now seats star DENTS as well as POINTS, so the Fig-3 dent-at-vertex
				// class is GENERATED by fill — recovered, no longer dropped. Positive marker; the loud flag
				// scopes the one case this work order does NOT cover.
				spikeBreak(
					'PeriodSolver.ts torusFill star gap-fill — DENT-seating ACTIVE (F5)',
					'the fill loop seats star DENTS (interior β, isotoxalDentAt) beside POINTS into the CW-most gap',
					'the Fig-3 dent-AT-vertex class (a reflex dent corner coincides with a real ≥3-tile vertex) is GENERATED by fill — star fill-reach lemma (F5), no longer a DROP',
					'STILL OUT OF SCOPE: a t=2 GEAR contact (a star point fills a star dent at a degree-2 NON-vertex) — not a busy-corner vertex; needs the gear-chain fill, not this work order',
				);
			} else {
				spikeBreak(
					'PeriodSolver.ts torusFill star gap-fill — DENT-seating off (point-only mode)',
					'the fill loop seats only star POINTS (interior α) into a gap',
					'the Fig-3 dent-AT-vertex class (a reflex dent corner coincides with a real ≥3-tile vertex) is NOT generated by fill in this mode ⇒ those tilings can be DROPPED (enable includeDents to recover them — star fill-reach lemma F5)',
					'POINT-ONLY mode (includeDents off): Fig-4 point-at-vertex is sound+complete; the dent-at-vertex class requires includeDents (digest-stable opt-in). Flag is loud, never silent (project doctrine)',
				);
			}
		}
