...
export const extractDataFromPolygonName = (name: string): PolygonSignatureData => {
    ...
    throw new Error(`Unrecognized polygon name: ${name}`);
}
...
const ENCODED_POLYGON_TOL = 1e-3;
/** Coarser grid for centroid bucketing so near-duplicates land in same bucket. */
const ROUND = 1e3;

/** Deduplicates encoded polygons (plain objects with vertices) for display. */
export const deduplicateEncodedPolygons = <T extends { vertices?: { x: number; y: number }[] }>(
    polygons: T[],
    tol: number = ENCODED_POLYGON_TOL
): T[] => {
    if (polygons.length <= 1) return polygons;
    const empty: T[] = [];
    const withVerts: T[] = [];
    for (const p of polygons) {
        if (!p.vertices || p.vertices.length === 0) empty.push(p);
        else withVerts.push(p);
    }
    if (withVerts.length <= 1) return [...empty, ...withVerts];

    const buckets = new Map<string, T[]>();
    for (const p of withVerts) {
        const [cx, cy] = centroidOf(p.vertices!);
        const key = `${Math.round(cx * ROUND)},${Math.round(cy * ROUND)}`;
        const list = buckets.get(key) ?? [];
        list.push(p);
        buckets.set(key, list);
    }
    ...
    for (const list of buckets.values()) {
        for (let i = 0; i < list.length; i++) {
            const p = list[i];
            const isFirst = list.findIndex((o) => encodedPolygonsEquivalent(p, o, tol)) === i;
            if (isFirst) result.push(p);
        }
    }
    return result;
};

function centroidOf(vertices: { x: number; y: number }[]): [number, number] {
    let cx = 0, cy = 0;
...
function encodedPolygonsEquivalent(
    a: { vertices?: { x: number; y: number }[] },
    b: { vertices?: { x: number; y: number }[] },
    tol: number
): boolean {
    const va = a.vertices ?? [];
    const vb = b.vertices ?? [];
    ...
    for (const v of va) {
        if (!vb.some((w) => Math.abs(v.x - w.x) < tol && Math.abs(v.y - w.y) < tol)) return false;
    }
    return true;
}
