...
// The third way an id can legitimately leave the shelf: shelf-dedup.py dropped it as contained in another
// entry. That pass writes no parameter map, so the redirect is id-only — but it is still a redirect, and
// an id in this table is not a lost tiling. Generated by scripts/gen-absorbed-aliases.mjs.
const ABSORBED: Record<string, unknown> = read("lib/services/absorbedFamilyAliases.json");
		...
		for (const r of WIDENED) {
			...
			if (!t) {
				// gone from the shelf: only legitimate if the widening made it a duplicate of another entry
					expect(
						ALIASES[r.id] ?? COUPLED_ALIASES[r.id] ?? ABSORBED[r.id],
						`${r.id} vanished without an alias`,
					).toBeDefined();
				continue;
			}
...
describe("resolveMergedFamilyKey", () => {
	...
	it("leaves an unknown or already-surviving key alone", () => {
		// k1-03 is a survivor in none of the three redirect tables. It used to be k2-58 here, which the
		// 2026-08 dedup pass absorbed into k2-54 — so that id now correctly redirects and no longer tests
		// the untouched path.
		const live = { tiling: "ctrnact-mixed-family-k1-03", alphas: [200] };
	...
	// The dedup pass drops an entry contained in another and writes no parameter map, so the id travels and
	// the angle does not. Each of these five was a live link before the 2026-08 mixed rebuild.
	it("redirects an absorbed id to its survivor, dropping the stale angle", () => {
		for (const [gone, survivor] of [
			["ctrnact-mixed-family-k2-03", "ctrnact-mixed-family-k2-02"],
			["ctrnact-mixed-family-k2-37", "ctrnact-mixed-family-k2-36"],
			["ctrnact-mixed-family-k2-39", "ctrnact-mixed-family-k2-36"],
			["ctrnact-mixed-family-k2-56", "ctrnact-mixed-family-k2-54"],
			["ctrnact-mixed-family-k2-58", "ctrnact-mixed-family-k2-54"],
		]) {
			expect(resolveMergedFamilyKey({ tiling: gone, alphas: [200] }), gone).toEqual({
				tiling: survivor,
				alphas: null,
			});
		}
